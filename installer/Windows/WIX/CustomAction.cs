...
namespace CustomActions
{
    public class CustomActions
    {
        ...
        {
            Microsoft.Deployment.WindowsInstaller.View view = session.Database.OpenView("SELECT * FROM ComboBox WHERE Property = 'IP_ADDRESS'");
            view.Execute();
        ...
        {
            Microsoft.Deployment.WindowsInstaller.View view = session.Database.OpenView("DELETE FROM ComboBox WHERE ComboBox.Property = 'IP_ADDRESS'");
            view.Execute();
        ...
        public static ActionResult Modify_PropertiesFile(Session session)
        {
            ...
            string path =  InstallFolder  + "Tomcat_Server\\webapps\\plugin-registration\\WEB-INF\\registerPlugin.properties";
            ...
            session.Log("FilePath :: " + path);
            try
            {           
                if (File.Exists(path))
                {
                    string rows = File.ReadAllText(path);
